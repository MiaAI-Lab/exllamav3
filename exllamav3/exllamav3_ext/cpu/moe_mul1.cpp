...
#endif
#else
// Non-x86 (ARM): no target attributes / intrinsics
#define M1_TARGET_AVX2
#define M1_TARGET_VNNI
#define M1_TARGET_VBMI
#endif
...
#else
// Scalar fallback (aarch64 etc.): never selected because g_isa is Scalar there, but kept
// functional in case of a mismatch
inline void hadamard_128_avx2(float* v) { hadamard_128_scalar(v); }
#endif
...
#else
// Scalar fallbacks (aarch64 etc.); only invoked if g_isa were not Scalar, which cannot happen
// on this hardware, but keep them functional. Mirror of the scalar branches in prepare_rows().
inline void quantize_row_avx2(const float* dst, int32_t* splat, int k, float& q_out, int32_t& s_out)
{
    float amax = 0.0f;
...
inline void prepare_block_avx2(const void* srcv, bool f16, const at::Half* suh, float* dst, int k)
{
    const float* srcf = f16 ? nullptr : static_cast<const float*>(srcv);
    const uint16_t* src16 = f16 ? static_cast<const uint16_t*>(srcv) : nullptr;
    for (int block = 0; block < k; block += 128)
    {
        float vals[128];
        for (int i = 0; i < 128; ++i)
        {
            const float xv = src16 ? half_to_float(at::Half(uint16_t(src16[block + i]), c10::Half::from_bits()))
                                   : srcf[block + i];
            vals[i] = xv * half_to_float(at::Half(uint16_t(suh[block + i]), c10::Half::from_bits()));
        }
        hadamard_128(vals);
        for (int i = 0; i < 128; ++i)
...
#if defined(__x86_64__) || defined(__i386__) || defined(_M_X64) || defined(_M_IX86)
// [aarch64 port] AVX-512 VNNI kernels
// -------------------------------------------------------------------------------------------
...
#endif // x86-only: AVX-512 VNNI kernels
#if defined(__x86_64__) || defined(__i386__) || defined(_M_X64) || defined(_M_IX86)
// [aarch64 port] AVX-512 VBMI kernels
// -------------------------------------------------------------------------------------------
...
#endif // x86-only: AVX-512 VBMI kernels
#if defined(__x86_64__) || defined(__i386__) || defined(_M_X64) || defined(_M_IX86)
// [aarch64 port] AVX2 kernels
// -------------------------------------------------------------------------------------------
//   AVX2
// -------------------------------------------------------------------------------------------
...
Isa detect_isa()
{
    ...
    #endif
#else
    // Non-x86 (ARM): only the scalar path is compiled
    hw = Isa::Scalar;
#endif
