...
// Portable equivalent of _mm_pause() (aarch64 "yield" hint)
...
// ---------------------------------------------------------------------------------------------
// BF16/FP16 accumulate kernels. AVX2/F16C implementations on x86, portable scalar
// bit-exact equivalents elsewhere (aarch64 etc.)
// ---------------------------------------------------------------------------------------------
...
// Scalar bit-exact mirror of the AVX2 BF16 add (integer arithmetic on the bit patterns)
inline void do16(uint16_t* __restrict ap, const uint16_t* __restrict bp)
{
    for (int j = 0; j < 16; ++j)
    {
        uint32_t s = (((uint32_t) ap[j]) << 16) + (((uint32_t) bp[j]) << 16);
        ap[j] = (uint16_t) ((s + 0x8000u) >> 16);
    }
}

static inline float bf16_half_to_float(uint16_t h)
{
...
static inline uint16_t float_to_fp16_rne(float f)
{
    uint32_t x;
    std::memcpy(&x, &f, 4);
    ...
    if (e >= -14)
    {
        uint32_t men = (m + 0x1000u + ((m >> 13) & 1)) >> 13;  // RNE to 10 mantissa bits
        if (men & 0x400u) { men >>= 1; e++; }
        if (e > 15) return (uint16_t) (sign | 0x7c00u);
        return (uint16_t) (sign | ((uint32_t) (e + 15) << 10) | (men & 0x3ffu));
    }
    // subnormal or zero, RNE
    uint32_t shift = (uint32_t) (-e - 14);
    uint32_t mm = m | 0x800000u;
    uint32_t s = mm >> shift;
    uint32_t rem = mm & ((1u << shift) - 1);
    uint32_t halfway = 1u << (shift - 1);
    if (rem > halfway || (rem == halfway && (s & 1))) s++;
    return (uint16_t) (sign | s);
}
...
inline void do16_fp16(uint16_t* __restrict ap, const uint16_t* __restrict bp)
{
    for (int j = 0; j < 16; ++j)
    {
        // fp16 wire stores raw fp16 bits (sign/exp/mantissa layout identical in bf16 container)
        float s = bf16_half_to_float(ap[j]) + bf16_half_to_float(bp[j]);
        ap[j] = float_to_fp16_rne(s);
    }
}
...
void enable_fast_fp_avx2()
{
    // No MXCSR on aarch64; nothing to do
}
#endif
