...
from .calibration_data import get_calibration
...
parser.add_argument("-cd", "--cal_data", type = str, default = "",
                    help = "Calibration trace file (JSON) to use instead of the default calibration corpus. "
                           "Accepts turboderp-style calibration traces (dict with 'rows' of input_ids/response_ids) "
                           "or a plain JSON list of token-ID lists")
...
def prepare(args) -> (dict, dict, bool, str):
    ...
    # cal_data must match on resume: switching calibration mid-job would quantize
    # later modules against a different distribution than earlier ones
    if args.resume and (in_args.get("cal_data") or "") != (args.cal_data or ""):
        raise ValueError(
            f" ## Error: Resuming job with cal_data = {in_args.get('cal_data')}, "
            f"cannot override with new value of {args.cal_data}. "
    ...
    if in_args.get("cal_data"):
        print(f"    Calibration trace: {in_args['cal_data']}")
...
def prepare_state(args, job_state, config, model, tokenizer):
    ...
    if idx == 0:
        ...
        state = get_calibration(args, tokenizer)
