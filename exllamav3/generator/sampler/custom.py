...
class SamplingState:
    ...
    state: SS = SS.INIT
    # When set, SS_Sample preserves a copy of the final proposal distribution before adding
    # Gumbel noise, for the lossless rejection-sampling verify in the generator
    ...
    def run(self, state: SamplingState):
        match state.state:
            ...
            case SS.LOGITS:
                ...
                state.sample = torch.argmax(state.logits, dim = -1)
            case SS.PROBS | SS.PROBS_N:
                if state.keep_probs: state.clean_probs = state.probs.clone()
...
class CustomSampler(Sampler):
    ...
    def unfused_equivalent(self) -> "CustomSampler":
        """A sampler with the same proposal distribution whose terminal step materializes
        probabilities, for the lossless rejection-sampling verify (EXL3_SPEC_RS). Rebuilds the
        step-by-step tail the fused step replaced; head penalty steps are shared as-is."""
        head = []
        fused = None
        for step in self.steps:
            if isinstance(step, SS_Fused):
                fused = step
            else:
                head.append(step)
        if fused is None:
            return self
        tail = []
        temperature = 1.0 / fused.inv_temp
        min_p = math.exp(fused.minp_log) if fused.minp_log else 0.0
        # temp_first is stored only via inv_temp_filter (= inv_temp when the temperature
        # precedes the filters, 1.0 otherwise); the distinction is moot at temperature 1.0
        temp_first = fused.inv_temp_filter != 1.0
        if temp_first:
            if temperature != 1.0: tail.append(SS_Temperature(temperature))
            if min_p: tail.append(SS_MinP(min_p))
            if fused.top_k: tail.append(SS_TopK(fused.top_k))
            if fused.top_p != 1.0: tail.append(SS_TopP(fused.top_p))
        else:
            if min_p: tail.append(SS_MinP(min_p))
            if fused.top_k: tail.append(SS_TopK(fused.top_k))
            if fused.top_p != 1.0: tail.append(SS_TopP(fused.top_p))
            if temperature != 1.0: tail.append(SS_Temperature(temperature))
        if fused.mode == SS_Fused.MODE_GREEDY:
            tail.append(SS_Argmax())
        else:
            tail.append(SS_Sample())
        # Bypass the automatic stack collapse, which would fold the reconstructed tail
        # straight back into an SS_Fused step
        global fused_sampler_enable
        _saved = fused_sampler_enable
        fused_sampler_enable = False
        try:
            return CustomSampler(head + tail)
        finally:
            fused_sampler_enable = _saved
